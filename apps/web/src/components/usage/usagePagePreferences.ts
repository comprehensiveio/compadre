...
export function readUsagePagePreferences(userId?: string): UsagePagePreferences {
  try {
    return (
      getLocalStorageItem(
        userId ? `${STORAGE_KEY}:user:${encodeURIComponent(userId)}` : STORAGE_KEY,
...
export function saveUsagePagePreferences(preferences: UsagePagePreferences, userId?: string): void {
  try {
    setLocalStorageItem(
      userId ? `${STORAGE_KEY}:user:${encodeURIComponent(userId)}` : STORAGE_KEY,
      preferences,
