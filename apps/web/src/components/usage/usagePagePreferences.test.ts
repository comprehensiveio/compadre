...
describe("Usage page preferences", () => {
  it("isolates authenticated users and does not inherit anonymous preferences", () => {
    saveUsagePagePreferences({ metric: "cost", windowDays: 1 });
    expect(readUsagePagePreferences("alice")).toEqual({ metric: "limits", windowDays: 30 });
    saveUsagePagePreferences({ metric: "tokens", windowDays: 7 }, "alice");
    expect(readUsagePagePreferences("bob")).toEqual({ metric: "limits", windowDays: 30 });
    saveUsagePagePreferences({ metric: "cost", windowDays: 90 }, "bob");
    expect(readUsagePagePreferences("alice")).toEqual({ metric: "tokens", windowDays: 7 });
    expect(readUsagePagePreferences()).toEqual({ metric: "cost", windowDays: 1 });
  });
