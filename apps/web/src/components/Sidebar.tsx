    ...
    <TooltipPopup side="right" align="start" sideOffset={4} variant="glass">
      ...
      <div className="flex min-w-0 max-w-80 flex-col gap-2 px-1 py-2">
        ...
        <div className="grid gap-1.5 pl-0.5 text-xs text-muted-foreground">
          {!COMPADRE_AUTH_ENABLED && projectDisplayName ? (
        ...
        >
          <div className="relative z-10 h-[4.875rem] px-(--sidebar-row-content-inset) py-(--sidebar-content-inset)">
            <div className="flex h-5 min-w-0 items-center gap-1.5">
              ...
              {!COMPADRE_AUTH_ENABLED && props.projectDisplayName ? (
                <span
                  className={cn(
            ...
            <div className="mt-0.5 flex min-w-0 items-center gap-1 text-secondary-label text-xs leading-none">
              ...
              <CompadrePreviewIndicator threadId={thread.id} environmentId={thread.environmentId} />
              {COMPADRE_AUTH_ENABLED && thread.externalThread ? (
                <CompadreSlackThreadLink reference={thread.externalThread} />
              ) : null}
            </div>
          </div>
          {props.jumpLabel ? <JumpHintBadge label={props.jumpLabel} /> : null}
...
export default function Sidebar() {
  ...
  const [identityFilter, setIdentityFilter] = useState<SidebarIdentityFilter>(
    COMPADRE_AUTH_ENABLED ? DEFAULT_SIDEBAR_IDENTITY_FILTER : "all",
  );
        ...
        fixedHeader={
          ...
          <SidebarGroup className="z-[1]">
            {COMPADRE_AUTH_ENABLED && currentCompadreUser ? (
              <CompadreConversationFilter value={identityFilter} onChange={setIdentityFilter} />
            ) : null}

            <SidebarThreadHeader
              searchFieldRef={headerSearchRef}
              hasProjects={!COMPADRE_AUTH_ENABLED && projectGroups.length > 0}
              projectScope={
                <Combobox
                  items={projectScopeItems}
