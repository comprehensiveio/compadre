...
export function isThreadDetailMissing(input: {
  threadId: ThreadId;
  status: "empty" | "cached" | "synchronizing" | "live" | "deleted";
  error: string | null;
}): boolean {
  return input.status === "deleted" || input.error === `Thread ${input.threadId} was not found`;
}

export function resolveThreadRouteRenderState(input: {
  bootstrapComplete: boolean;
  serverThreadDetailExists: boolean;
  serverThreadDetailMissing: boolean;
  ...
  if (input.serverThreadDetailMissing) {
    return "missing";
  }
  return "loading";
}
