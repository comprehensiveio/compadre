...
export async function restoreRejectedQueuedMessage(
  ...
  try {
    ...
    if (existingAttachmentIds.size + addedAttachmentCount > MOBILE_COMPOSER_MAX_ATTACHMENTS) {
      setPendingConnectionError(
        `Remove attachments from the draft before restoring this message. Messages can contain at most ${MOBILE_COMPOSER_MAX_ATTACHMENTS} attachments.`,
      );
      return "blocked";
    }
