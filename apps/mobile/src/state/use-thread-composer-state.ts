  ...
  if (input.attachments && input.attachments.length > 0) {
    ...
    if (rejectedCount > 0) {
      setPendingConnectionError(
        `${rejectedCount} comment attachment${rejectedCount === 1 ? " was" : "s were"} not added. Messages can contain at most ${MOBILE_COMPOSER_MAX_ATTACHMENTS} attachments.`,
      );
    }
  }
}
...
export function useThreadComposerState() {
  ...
  const onSendMessage = useCallback(async () => {
    ...
    if (attachments.length > MOBILE_COMPOSER_MAX_ATTACHMENTS) {
      Alert.alert(
        ...
        `Remove attachments until there are at most ${MOBILE_COMPOSER_MAX_ATTACHMENTS}.`,
      );
      return null;
    }
  ...
  const onPickDraftFiles = useCallback(async () => {
    ...
    const problems = [
      ...(result.error ? [result.error] : []),
      ...(rejectedCount > 0
        ? [`You can attach up to ${MOBILE_COMPOSER_MAX_ATTACHMENTS} files per message.`]
        : []),
    ];
    if (problems.length > 0) {
