...
describe("mobile composer drafts", () => {
  ...
  it.each(["attachment", "context", "imported context"])(
    ...
    async (kind) => {
      ...
      const files = Array.from({ length: MOBILE_COMPOSER_MAX_ATTACHMENTS }, (_, index) => ({
        id: `file-${index}`,
        type: "file" as const,
      ...
      expect(countComposerDraftAttachmentsAfterSelection(key, insertion)).toBe(
        MOBILE_COMPOSER_MAX_ATTACHMENTS - 1,
      );
      ...
      expect(draft.attachments.slice(0, MOBILE_COMPOSER_MAX_ATTACHMENTS - 1)).toEqual(
        files.slice(1),
      );
  ...
  it("releases videos rejected by the live draft limit and keeps accepted files", async () => {
    ...
    const existing = Array.from({ length: MOBILE_COMPOSER_MAX_ATTACHMENTS - 1 }, (_, index) =>
    ...
    expect(draft?.attachments).toHaveLength(MOBILE_COMPOSER_MAX_ATTACHMENTS);
  ...
  it("preserves existing images when shared content exceeds the draft attachment limit", () => {
    ...
    const sharedImages = Array.from({ length: MOBILE_COMPOSER_MAX_ATTACHMENTS }, (_, index) =>
      image(`shared-${index}`),
    );
    ...
    expect(merged[draftKey]?.attachments).toHaveLength(MOBILE_COMPOSER_MAX_ATTACHMENTS);
    expect(merged[draftKey]?.attachments[0]).toEqual(existingImage);
    expect(merged[draftKey]?.attachments.at(-1)?.id).toBe(
      `shared-${MOBILE_COMPOSER_MAX_ATTACHMENTS - 2}`,
    );
  });
