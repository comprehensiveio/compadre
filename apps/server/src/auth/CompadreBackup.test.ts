...
describe("Compadre SQLite backup", () => {
  it("requires a dedicated backup credential", () => {
    ...
    NodeAssert.equal(
      configuredBackupToken({ COMPADRE_BACKUP_TOKEN: "  backup-key  " }),
      "backup-key",
    );
  });
  ...
  it.effect("creates a consistent standalone copy of a WAL database", () =>
    Effect.gen(function* () {
      ...
      const directory = path.join("/tmp", `compadre-t3-backup-${NodeCrypto.randomUUID()}`);
      ...
      try {
        ...
        NodeAssert.ok(artifact.bytes.byteLength > 0);
        NodeAssert.match(artifact.sha256, /^[a-f0-9]{64}$/);
        const restoredPath = path.join(directory, "restored.sqlite");
        yield* fs.writeFile(restoredPath, artifact.bytes);
        const restored = new NodeSqlite.DatabaseSync(restoredPath, { readOnly: true });
        try {
          const row = restored.prepare("SELECT text FROM messages").get() as {
            text: string;
          };
          NodeAssert.equal(row.text, "durable message");
        } finally {
          restored.close();
        }
      } finally {
