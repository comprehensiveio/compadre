    ...
    writeSemaphore.withPermits(1)(
      Effect.gen(function* () {
        if (
          ...
          (patch.storageCleanup !== undefined ||
            patch.worktreeCleanup !== undefined ||
            Object.values(patch.projectSettingsOverrides ?? {}).some(
              (override) => override?.worktreeCleanup !== undefined,
            ))
        ) {
          return yield* new ServerSettingsError({
            settingsPath,
            operation: "normalize",
            cause: "Storage cleanup is managed by the Compadre controller and Modal workers.",
          });
        }
        ...
        if (
          process.env.COMPADRE_NATIVE_T3_URL &&
          Object.values(updated.providerInstances).some(
            (instance) =>
          ...
          return yield* new ServerSettingsError({
            settingsPath,
            operation: "normalize",
            cause: "Hosted Compadre supports only Codex and Claude provider instances.",
          });
        }
