...
const makeProjectionThreadMessageRepository = Effect.gen(function* () {
  ...
  const upsertProjectionThreadMessageRow = SqlSchema.void({
    ...
    execute: (row) => {
      ...
      const nextAttributionJson =
        row.attribution !== undefined ? JSON.stringify(row.attribution) : null;
      ...
      return sql`
        ...
        DO UPDATE SET
          ...
          attribution_json = COALESCE(
            excluded.attribution_json,
            projection_thread_messages.attribution_json
          ),
    ...
    execute: (row) => {
      ...
      return sql`
        ...
        DO UPDATE SET
          ...
          attribution_json = COALESCE(excluded.attribution_json, projection_thread_messages.attribution_json),
          context_json = COALESCE(
            excluded.context_json,
            projection_thread_messages.context_json
          ),
  ...
  const listUserAttributionRows = SqlSchema.findAll({
    Request: ListProjectionThreadMessagesInput,
    Result: Schema.Struct({ attribution: Schema.fromJsonString(MessageAttribution) }),
    execute: ({ threadId }) => sql`
      SELECT attribution_json AS attribution FROM projection_thread_messages
      WHERE thread_id = ${threadId} AND role = 'user' AND attribution_json IS NOT NULL
  ...
  const getLatestUserMessageAtRow = SqlSchema.findOne({
    ...
    execute: ({ threadId }) => sql`
      ...
      WHERE thread_id = ${threadId} AND role = 'user'
        AND substr(message_id, 1, 7) <> 'import:'
    `,
  });
  ...
  return {
    ...
    listByThreadId,
    listUserAttributions: (input) =>
      listUserAttributionRows(input).pipe(
        Effect.map((rows) => rows.map((row) => row.attribution)),
        Effect.mapError(
          toPersistenceSqlError("ProjectionThreadMessageRepository.listUserAttributions:query"),
        ),
      ),
