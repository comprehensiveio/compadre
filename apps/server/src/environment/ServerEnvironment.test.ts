...
it.layer(NodeServices.layer)("ServerEnvironmentLive", (it) => {
  ...
  it.effect("does not advertise features without hosted ownership adapters", () =>
    Effect.gen(function* () {
      vi.stubEnv("COMPADRE_NATIVE_T3_URL", "http://controller.invalid");
      const fs = yield* FileSystem.FileSystem;
      const baseDir = yield* fs.makeTempDirectoryScoped({ prefix: "hosted-capabilities-" });
      const descriptor = yield* Effect.gen(function* () {
        return yield* (yield* ServerEnvironment.ServerEnvironment).getDescriptor;
      }).pipe(Effect.provide(makeServerEnvironmentLayer(baseDir)));
      expect(descriptor.capabilities.questionAttachments).toBe(false);
      expect(descriptor.capabilities.threadActiveReorder).toBe(false);
      expect(descriptor.capabilities.threadAutoSettleOptOut).toBe(false);
      expect(descriptor.capabilities.storageCleanup).toBe(false);
      expect(descriptor.capabilities.projectWorktreeCleanup).toBe(false);
      expect(descriptor.capabilities.pullRequestStackActions).toBe(false);
      expect(descriptor.capabilities.threadPullRequests).toBe(true);
    }),
  );
