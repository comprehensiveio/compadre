...
it.layer(NodeServices.layer)("server settings", (it) => {
  it.effect("rejects hosted cleanup settings without changing persisted policy", () =>
    Effect.gen(function* () {
      ...
      for (const patch of [
        { storageCleanup: { browserArtifactsAfterDays: 1 } },
        {
          providerInstances: {
        ...
        const error = yield* settings.updateSettings(patch).pipe(Effect.flip);
