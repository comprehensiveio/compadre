...
export const directTerminalRouteLayer = HttpRouter.add(
  ...
  Effect.gen(function* () {
    ...
    const close = Effect.gen(function* () {
      closed = true;
      detach?.();
      yield* write.write(new Socket.CloseEvent(1008, "Terminal connection ended"));
    });
    ...
    yield* Stream.runForEach(
      Stream.fromPull(
        Effect.map(socket.reader, (reader) =>
          Effect.map(reader.pull, (chunks) =>
            Arr.map(chunks, (chunk) =>
        ...
        return serial
          .withPermit(
            Effect.gen(function* () {
              ...
              yield* write.write(encode({ type: "ack", id: message.id }));
            }),
          )
          .pipe(
            Effect.catch(() => close),
