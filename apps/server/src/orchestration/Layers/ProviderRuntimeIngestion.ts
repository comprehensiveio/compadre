...
export function runtimeEventToActivities(
  ...
  switch (event.type) {
    case "turn.completed": {
      ...
      // the visible transcript. The activity is also the durable terminal
      // signal sent over T3 Connect, so it must exist for ordinary successful
      // turns as well as turns carrying a special stop reason. Hosted
      // coordinators use the optional reason to reject truncated answers.
      return [
        {
          id: event.eventId,
...
const make = Effect.gen(function* () {
  ...
  const recordProviderDiff = Effect.fn("recordProviderDiff")(function* (event: ProviderDiffEvent) {
    ...
    if (!event.payload.savedReview && (Option.isNone(turn) || turn.value.state !== "running"))
      return;
    ...
    if (existing && (!event.payload.savedReview || existing.status === "ready")) return;
    const now = event.createdAt;
    yield* orchestrationEngine.dispatch({
      ...
      checkpointRef: CheckpointRef.make(
        event.payload.savedReview?.reference ?? `provider-diff:${event.eventId}`,
      ),
      status: event.payload.savedReview ? "ready" : "missing",
      files: event.payload.savedReview?.files ?? [],
      assistantMessageId: MessageId.make(
        `assistant:${event.itemId ?? event.turnId ?? event.eventId}`,
      ),
      checkpointTurnCount:
        existing?.checkpointTurnCount ?? maxCheckpointTurnCount(checkpointContext.checkpoints) + 1,
      createdAt: now,
    });
  });
    ...
    processInput(input).pipe(
      ...
      Effect.catchCause((cause) => {
        ...
        return Effect.logWarning("provider runtime ingestion failed to process event", {
          ...
          threadId: input.source !== "domain" ? input.event.threadId : input.event.payload.threadId,
          cause: Cause.pretty(cause),
        });
      }),
    );
  ...
  const worker = yield* makeFairDrainableWorker({
    key: (input: RuntimeIngestionInput) =>
      input.source !== "domain" ? input.event.threadId : input.event.payload.threadId,
  ...
  const diffWorker = yield* makeFairDrainableWorker({
    key: (event: ProviderDiffEvent) => event.threadId,
    process: (event: ProviderDiffEvent) =>
