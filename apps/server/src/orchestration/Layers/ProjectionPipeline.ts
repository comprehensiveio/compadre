...
const makeOrchestrationProjectionPipeline = Effect.fn("makeOrchestrationProjectionPipeline")(
  function* () {
    ...
    const refreshThreadShellSummary = Effect.fn("refreshThreadShellSummary")(function* (
      ...
      const attributions = yield* projectionThreadMessageRepository.listUserAttributions({
        threadId,
      });
    ...
    const applyThreadsProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        // Actor-bearing user messages also update participation. Assistant deltas
        // retain upstream's incremental summary path without scanning the transcript.
        case "thread.message-sent": {
          if (event.payload.role === "user" && event.payload.attribution !== undefined) {
    ...
    const applyThreadMessagesProjection: ProjectorDefinition["apply"] = Effect.fn(
      ...
      switch (event.type) {
        ...
        case "thread.message-sent": {
          ...
          const nextAttribution = event.payload.attribution ?? previousMessage?.attribution;
          yield* projectionThreadMessageRepository.upsert({
            ...
            ...(nextAttribution !== undefined ? { attribution: nextAttribution } : {}),
