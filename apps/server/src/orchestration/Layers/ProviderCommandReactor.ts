...
const make = Effect.gen(function* () {
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    // Hosted compaction is a durable controller action, replayed by the Modal
    // worker. The local adapter's in-process compaction cannot own that request.
    const isCompactCommand =
      !process.env.COMPADRE_NATIVE_T3_URL && isCompactCommandMessage(message);
