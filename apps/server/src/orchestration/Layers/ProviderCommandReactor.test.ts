...
describe("ProviderCommandReactor", () => {
  ...
  it("reacts to thread.turn.start by ensuring session and sending provider turn", async () => {
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        message: {
          ...
          role: "user",
          text: "hello reactor",
          providerPrompt: "hello reactor with Slack routing context",
          attachments: [],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
    ...
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      input: "hello reactor with Slack routing context",
    });
  ...
  effectIt.effect("routes hosted compaction through the durable native provider action", () =>
    Effect.gen(function* () {
      ...
      yield* harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.make("cmd-hosted-compact"),
        threadId: ThreadId.make("thread-1"),
        message: {
          messageId: asMessageId("user-hosted-compact"),
          role: "user",
          text: "/compact",
          attachments: [],
        },
      ...
      expect(harness.sendTurn).toHaveBeenCalledWith(
        expect.objectContaining({
          providerAction: expect.objectContaining({ type: "compact" }),
        }),
      );
    }),
  );
