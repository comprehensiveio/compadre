import { HostedReviewViewer } from "../compadre/HostedReviewViewer.ts";
...
it.effect(
  "keeps hosted viewed files personal with a shared provider account and supports unticking",
  () =>
    Effect.gen(function* () {
      const previous = process.env.COMPADRE_NATIVE_T3_URL;
      process.env.COMPADRE_NATIVE_T3_URL = "http://controller.invalid";
      ...
      const provider = environmentViewedProvider(new Map([["src/a.ts", "revision-1"]]), []);
      const service = yield* makeService({
        projects: [
          project({
            id: "p1",
      ...
      const write = (viewed: boolean) =>
        service.setFilesViewed({ ...GITLAB_REFERENCE, files: [{ path: "src/a.ts", viewed }] });
      yield* write(true).pipe(Effect.provideService(HostedReviewViewer, "alice"));
      assert.deepStrictEqual(
        (yield* service
          .filesViewed(GITLAB_REFERENCE)
          .pipe(Effect.provideService(HostedReviewViewer, "bob"))).files,
        [],
      );
      assert.deepStrictEqual(
        (yield* service
          .filesViewed(GITLAB_REFERENCE)
          .pipe(Effect.provideService(HostedReviewViewer, "alice"))).files,
        [{ path: "src/a.ts", state: "viewed" }],
      );
      yield* write(false).pipe(Effect.provideService(HostedReviewViewer, "bob"));
      assert.strictEqual(
        (yield* service
          .filesViewed(GITLAB_REFERENCE)
          .pipe(Effect.provideService(HostedReviewViewer, "alice"))).files.length,
        1,
      );
      yield* write(false).pipe(Effect.provideService(HostedReviewViewer, "alice"));
      assert.deepStrictEqual(
        (yield* service
          .filesViewed(GITLAB_REFERENCE)
          .pipe(Effect.provideService(HostedReviewViewer, "alice"))).files,
      ...
      assert.strictEqual((yield* Effect.flip(write(true)))._tag, "PullRequestOperationError");
    }),
);
