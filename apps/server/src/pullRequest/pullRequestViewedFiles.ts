...
export const make = (dependencies: Dependencies) => {
  ...
  const viewerOf = (project: SupportedProject, operation: string) =>
    Effect.gen(function* () {
      if (!process.env.COMPADRE_NATIVE_T3_URL) return yield* requiredViewerOf(project, operation);
      const viewer = yield* HostedReviewViewer;
      if (viewer === null)
        return yield* new PullRequestOperationError({
          operation,
          detail: "Viewed files require an authenticated Compadre user.",
        });
      return `compadre:${viewer}`;
    });
  /**
  ...
  const environmentFilesViewed = (
    ...
    Effect.gen(function* () {
      const viewer = yield* viewerOf(project, "filesViewed");
  ...
  const environmentSetFilesViewed = (
    ...
    Effect.gen(function* () {
      const viewer = yield* viewerOf(project, "setFilesViewed");
  ...
  const filesViewed = (input: PullRequestRef) =>
    requireProject(input).pipe(
      Effect.flatMap((project): Effect.Effect<PullRequestFilesViewedResult, PullRequestError> => {
        if (process.env.COMPADRE_NATIVE_T3_URL) return environmentFilesViewed(project, input);
