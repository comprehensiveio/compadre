...
import { buildClaudeMcpServers, readCompadreMcpBridge } from "../../mcp/CompadreMcpBridge.ts";
...
export const makeClaudeAdapter = Effect.fn("makeClaudeAdapter")(function* (
  ...
  const handleResultMessage = Effect.fn("handleResultMessage")(function* (
    ...
    const missingActionReceipt =
      turn?.providerAction?.type === "compact" && !turn.compactionObserved;
    const status = missingActionReceipt ? "failed" : outcome.status;
    const errorMessage =
  ...
  const startSession: ClaudeAdapterShape["startSession"] = Effect.fn("startSession")(
    function* (input) {
      ...
      // Modal runs T3 as root; Claude rejects bypassPermissions for that user.
      // The full-access tool policy still lives in canUseTool.
      const permissionMode =
        effectiveUserId === 0 && requestedPermissionMode === "bypassPermissions"
          ? "acceptEdits"
      ...
      const mcpServers = buildClaudeMcpServers(mcpSession, compadreMcpBridge);
