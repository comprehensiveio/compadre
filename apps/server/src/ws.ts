      ...
      return WsRpcGroup.of({
          ...
          observeRpcEffect(
            ...
            Effect.gen(function* () {
              ...
              const normalizedCommand = yield* normalizeDispatchCommand(
                attributeCompadreWebCommand(command, currentSession.subject),
              );
        ...
        [WS_METHODS.pullRequestsFilesViewed]: (input) =>
          observeRpcEffect(
            ...
            withPullRequestViewer(input, pullRequests.filesViewed(input)).pipe(
              Effect.provideService(
                HostedReviewViewer,
                decodeCompadreUserSubject(currentSession.subject)?.id ?? null,
              ),
            ),
            { "rpc.aggregate": "pull-requests" },
          ),
        [WS_METHODS.pullRequestsSetFilesViewed]: (input) =>
          observeRpcEffect(
            ...
            withPullRequestViewer(input, pullRequests.setFilesViewed(input)).pipe(
              Effect.provideService(
                HostedReviewViewer,
                decodeCompadreUserSubject(currentSession.subject)?.id ?? null,
              ),
        ...
        [WS_METHODS.gitRunStackedAction]: (input) =>
          observeRpcStream(
            ...
            process.env.COMPADRE_NATIVE_T3_URL
              ? Stream.fail(
                  new GitManagerError({
                    operation: "gitRunStackedAction",
                    cwd: input.cwd,
                    detail: "PR stack actions require a Modal execution adapter.",
                  }),
                )
