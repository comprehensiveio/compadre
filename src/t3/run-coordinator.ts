 ...
 * `start` holds a distributed per-run lock and writes to Postgres, while HTTP
 * readers independently tail `stream`. A subscriber disconnect therefore
 * cannot cancel billed work. `cancel` records durable intent before using the
...
export class NativeT3RunCoordinator {
  ...
  async start(input: NativeT3RunStart): Promise<NativeT3RunStartResult> {
    ...
    const drive = this.locks.withLock(
      `compadre:native-t3-run:${input.runId}`,
      async (lockSignal) => {
