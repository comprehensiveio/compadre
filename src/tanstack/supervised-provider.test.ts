...
test("reports spawned process roots without changing process behavior", async () => {
  ...
  try {
    const process = await handle.process.spawn("printf supervised");
    let stdout = "";
    for await (const chunk of process.stdout) stdout += chunk;
    assert.equal(await process.wait(), 0);
    assert.equal(stdout, "supervised");
    assert.deepEqual(pids, [process.pid]);
  } finally {
