...
| Official Slack app identity and delivery | Proven live | The official `Compadre` app owns production events, shows active tools as Slack status, withholds web-only narration, and posts exactly one final answer plus one canonical open session in Compadre web link. A clean redeploy proved that its credentials come from the canonical Render env group rather than service overrides. |
...
| Restart takeover | Implemented for native provider turns: a replacement controller reattaches to the existing Modal T3 worker snapshot, reprojects narration and detailed tools without redispatching, and fences stale event/run writes with a durable driver epoch. Slack completion remains independently durable through its Postgres outbox. | Keep a rolling-deploy canary that proves rich transcript recovery and exactly one final Slack delivery. |
