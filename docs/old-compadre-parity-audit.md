...
| Temporary Slack app identity and delivery | Proven | A deployed instance authenticated as `Secret dre experiment`, showed the active shell tool as Slack status, withheld web-only narration, and posted exactly one final answer plus one Open in web link under that identity. |
...
| Slack answer ownership | The automatic owner now posts only T3's designated final assistant message, and Slack-origin turns receive hidden instructions not to self-deliver. The harness can still call a same-thread Slack tool and duplicate the answer because that rule is not structurally enforced. | Reject or deduplicate same-thread final delivery in the Slack tool bridge; keep prompting as defense in depth. |
...
| Restart takeover | A controller restart can replay events but cannot reattach and continue projection. A live rolling-deploy probe also showed that an in-memory Slack completion callback can disappear while the central turn continues. | Persist dispatch and Slack-delivery jobs with heartbeat state; add idempotent claiming, leases, and epoch fencing. |
