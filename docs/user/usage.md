...
In hosted Compadre, Codex uses the shared ChatGPT subscription whenever its single managed lane is
available. While one Codex run owns that lane, other concurrent Codex runs use API billing and the
Limits page explains that the live subscription balance cannot be checked. The page also reports
when subscription routing is disabled or when the account check fails, instead of treating those
states as though no subscription exists.
