...
durable producer continues) before opening the steering stream. A browser steer
makes the combined answer UI-only: an existing Slack outbox owner settles
without posting and clears its processing status. A racing Slack steer can
reserve another durable outbox row before central running state is visible;
older owners yield to that replacement, which posts the final answer and clears
the shared status exactly once. Yielding observers must cancel their local Slack
processing refresh timers without writing a terminal status; otherwise a stale
timer can resurrect the working indicator after the final owner has cleared it.
