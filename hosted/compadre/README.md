...
npm install
npm run temporal:up          # local Temporal server (gRPC :7243, UI :8243)
npm run dev                  # tsx watch on port 3100
npm run test:thread-persistence # database-free two-turn persistence regression
npm run temporal:probe       # end-to-end durable-run proof against local Temporal
```
...
- **POSTHOG_MCP_URL / POSTHOG_MCP_MODE**: The endpoint defaults to `https://mcp.posthog.com/mcp`, mode defaults to token-efficient `cli`, and access is always write-capable so agents can create and update insights and dashboards in the pinned project. The Comprehensive organization and project IDs are non-secret constants in `src/mcp.ts`; their headers remove context-switching tools.
...
- **MODAL_TOKEN_ID / MODAL_TOKEN_SECRET**: Modal is the coding-harness runtime. Compadre bakes its pinned Claude Code and Codex CLIs into the cached Modal image, keeping per-request setup to the repository clone. A hosted-T3 worker remains live until Modal's hard lifetime (two hours by default), with a live filesystem checkpoint after each terminal turn. The next message restores a new sandbox from that snapshot after the worker expires. There is currently no shorter post-turn warm lease or sweeper.
...
Native Node.js service. Build: `npm ci --include=dev && npm run build` (also
compiles the Temporal workflow bundle), Start: `npm start`.

The production `compadre-api` controller, `compadre-web` T3 fork, and the
`compadre-temporal` orchestration server (plus its `compadre-temporal-db`
Postgres) are declared
...
lifecycle, and recovery metadata. A self-hosted Temporal server orchestrates
each native T3 run as a durable workflow, so controller restarts resume active
runs instead of orphaning them (see
[Temporal orchestration](docs/temporal-orchestration.md)). Codex or Claude Code, the repository, shell,
