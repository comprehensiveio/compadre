...
export async function publishNativeRunOutputs(input: {
  gateway: Pick<T3Gateway, "attachWorker" | "captureWorkspaceReview" | "collectOutputArtifacts">; artifactStore: T3ArtifactStore; reviews: WorkspaceReviewStore | null;
  turn: T3GatewayTurn; request: NativeT3RunRequest; metadata: MetadataStore;
  checkpoint?: ReturnType<typeof nativeOutputCheckpoints>[number];
}): Promise<void> {
  ...
  const turnId = input.checkpoint?.turnId ?? snapshot.thread.latestTurn?.turnId;
  if (!turnId) return;
  ...
  const checkpoint = input.checkpoint ?? reviewCheckpointForMessage(finished, turn.dispatch.messageId);
