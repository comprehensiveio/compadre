...
import { sandboxCompadreSkillPath } from "../compadre-skills.js";
...
- PostHog: product analytics, dashboards, feature flags, experiments, session replay, and error tracking; access is read-only unless the controller is explicitly configured for writes
...
## Managed Comp development server
When the user asks to start, run, open, serve, preview, or tunnel the Comp application, treat that as a request for the complete managed development environment even if they do not separately ask for a public URL. Before running any server command, invoke /compadre:dev-environment and follow it. Run \`scripts/compadre-dev-up.sh up\`; do not launch the framework dev server directly or create another tunnel with cloudflared, ngrok, localtunnel, or a similar tool.

Managed startup always exposes the sandbox's stable public preview. Finish the request by proactively returning the skill's complete Comp dev-login URL on \`*.dev.compadre.comprehensive.io\`, choosing the most relevant sandbox-local user and page as the skill describes. Do not return a bare preview origin, localhost URL, \`*.modal.host\` URL, \`*.trycloudflare.com\` URL, or \`auto_impersonate\` link.
...
- query-database: \`${sandboxCompadreSkillPath("query-database")}\`
- pull-request: \`${sandboxCompadreSkillPath("pull-request")}\`
- integration-debugging: \`${sandboxCompadreSkillPath("integration-debugging")}\`
- dev-environment: \`${sandboxCompadreSkillPath("dev-environment")}\`
...
- /compadre:dev-environment — BEFORE starting, validating, or sharing the isolated Comp development environment. The managed development-server rule above applies whenever the user asks to start the app. When sharing it, proactively return the skill's full Comp dev-login URL for the sandbox-local user whose role, permissions, company, and data best demonstrate the functionality discussed in the thread, not a bare preview URL or an \`auto_impersonate\` link. Only when the thread provides too little context to choose a meaningful user, default to the admin login for company ID 9.
