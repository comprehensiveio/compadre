...
- /compadre:dev-environment — BEFORE starting, validating, or sharing the isolated Comp development environment. When sharing it, proactively return the skill's full Comp dev-login URL for the sandbox-local user whose role, permissions, company, and data best demonstrate the functionality discussed in the thread, not a bare preview URL or an \`auto_impersonate\` link. Only when the thread provides too little context to choose a meaningful user, default to the admin login for company ID 9.
